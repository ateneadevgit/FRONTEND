...
@Component({
  selector: 'app-doc-guidelines',
  templateUrl: './doc-guidelines.component.html',
  styleUrls: ['./doc-guidelines.component.scss'],
})
export class DocGuidelinesComponent implements OnInit {
  ...
  formDocument = this.fb.group({
    ...
    order: new FormControl(0, [Validators.pattern('^[0-9]*$'), this.greaterThanZero]),
  });
  ...
  getGuidelines() {
    this.documentService.getDocuments(GuidelineTypeDoc.GUIDELINE).subscribe((response) => {
  ...
  createGuideline() {
    if (
      ...
      this.formDocument.get('order')?.invalid ||
      !this.documentBase64
    ) {
      this.alertService.showErrorMessage({
        title: 'Error',
      ...
      const newDocument: IDocGuidelineRequest = {
        ...
        fileType: GuidelineTypeDoc.GUIDELINE,
      };
  ...
  cleanForm() {
    ...
    this.formDocument.controls.order.setValue(null);
