...
import { IAreaColor, IDataMyProgress, IMyProgramCourse } from 'src/models/dashboard.inteface';
  ...
  loadData() {
    if (this.programSelect) {
      this.dashboardService
        .getHistorySubject(this.programSelect.idProgram)
        .subscribe((response) => {
          this.listCourse = response.data;
          this.listCourse = this.listCourse.sort((a, b) => a.coreId - b.coreId);
          this.areaList = this.organizeAndGenerateColors(this.listCourse);
        });

      this.dashboardService
        .getProgramProgress(this.programSelect.idProgram)
        .subscribe((response) => {
